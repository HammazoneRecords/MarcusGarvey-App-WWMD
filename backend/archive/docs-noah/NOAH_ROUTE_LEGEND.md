...
### tools/solob.ps1
- Role: ergonomic wrapper for:
  - `record -note "..."` (state transition with confirmation latch)
  - `observe -note "..."` (seal)
  - `run -intent "..." -script <script> -args @(...)` (recorded execution)
- Guarantees: still routes through `scripts/run_recorded.py` and state guard.
