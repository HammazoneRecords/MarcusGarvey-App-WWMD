...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Extended post-ingestion audit (chunks allowed, sanity enforced).")
    ...
    ap.add_argument("--require-runs-zero", action="store_true", default=True)
    ...
    # Manifest still MUST match disk (anchors are your evidence base)
    ...
    try:
        ...
        if counts["chunks"] < args.min_chunks:
            raise SystemExit(f"AUDIT FAILED: chunks={counts['chunks']} expected >= {args.min_chunks}")

        if args.require_runs_zero and counts["runs"] != 0:
            raise SystemExit(f"AUDIT FAILED: runs={counts['runs']} expected 0 (post-ingest still should be runs=0)")

        # Duplicate guards (your manual checks, baked in)
        dup_chunk_id = conn.execute(
            "select count(*) from (select chunk_id from chunks group by chunk_id having count(*)>1)"
        ).fetchone()[0]
        dup_locator = conn.execute(
            "select count(*) from (select anchor_id, anchor_locator from chunks group by anchor_id, anchor_locator having count(*)>1)"
        ).fetchone()[0]
        if dup_chunk_id != 0:
            raise SystemExit(f"AUDIT FAILED: duplicate chunk_id groups={dup_chunk_id}")
        if dup_locator != 0:
            raise SystemExit(f"AUDIT FAILED: duplicate (anchor_id, anchor_locator) groups={dup_locator}")

        # Orphan guard: every chunk must reference a real anchor_id
        ...
        if orphan_chunks != 0:
            raise SystemExit(f"AUDIT FAILED: orphan chunks (anchor_id not in anchors) count={orphan_chunks}")
    ...
    report: Dict[str, Any] = {
        ...
        "dup_chunk_id_groups": int(dup_chunk_id),
        "dup_locator_groups": int(dup_locator),
    ...
    print("POST-INGESTION AUDIT EXT: PASS")
    print(f"OK: counts={counts} fk={fk} integrity={integ}")
    print(f"OK: dup_chunk_id_groups={dup_chunk_id} dup_locator_groups={dup_locator} orphan_chunks={orphan_chunks}")
    print(f"OK: manifest={report['manifest_rel']} sha256={report['manifest_sha256']}")
    return 0
